...
Tavola project specs describe the generated application and remain supported as
a compatibility input, review artifact, and fixture format. The Go generator can
also generate directly from sqlmeta metadata and expanded app intent without
writing a Tavola JSON spec first.
...
compatibility example. The template contains a small app with a public role
`p`, a non-public login role `u`, a login table, one login procedure, and one
protected component.

For apps generated from an existing database, use `cmd/tavola-generate` with
sqlmeta inputs. See [`sqlmeta-introspection.md`](sqlmeta-introspection.md).
...
contract. Use `goFilterFile` and `goModelFile` for Go component overlays, and
keep those overlay source files outside any output directory regenerated with
`--replace`.
`componentJson` and
...
Specs and generated API manifests produced from sqlmeta include:
