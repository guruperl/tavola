...
sub before {
  ...
  if ($who eq 'admin' && $action eq 'topics' && $ARGS->{u}) {
  ...
  if ($action eq 'topics') {
    ...
    $extra->{memberid} = $ARGS->{memberid} if (($who eq 'admin') && $ARGS->{memberid});
    $extra->{status} = $ARGS->{status} if ($ARGS->{status});
  } elsif ($action eq 'reply') {
    $extra->{memberid} = $ARGS->{memberid} if ($who eq 'member');
  }
